        ...
        implements java.io.Serializable
{
    ...
    public Matcher matcher(String input) {
        return matcher(new StringCharSequence(input));
    }
    ...
    private void compile() {
        ...
        for (int x = 0; x < patternLength; x += 1) {
            temp[count++] = normalizedPattern.charAt(x);
        }
    ...
    {
        throw new RuntimeException("Unicode uh oh");
    }
...
//            assert (index >= 0 && index < length);
    ...
    private CharProperty newSingle(final int ch) {
        if (has(CASE_INSENSITIVE)) {
            int lower, upper;
            if (has(UNICODE_CASE)) {
                upper = Character.toUpperCase((char)ch);
                lower = Character.toLowerCase((char)upper);
                if (upper != lower)
            ...
            if (has(UNICODE_CASE)) {
                for (int i = 0; i < count; i++) {
                    tmp[i] = Character.toLowerCase(
                            Character.toUpperCase((char)buf[i]));
                }
    ...
    static final class SliceU extends SliceNode {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            for (int j=0; j<len; j++) {
                ...
                int c = seq.charAt(i+j);
                if (buf[j] != c &&
                        buf[j] != Character.toLowerCase(Character.toUpperCase((char) c)))
                    return false;
            }
            return next.match(matcher, i+len, seq);
        }
    ...
    static final class SliceUS extends SliceIS {
        ...
        int toLower(int c) {
            return Character.toLowerCase(Character.toUpperCase((char)c));
        }
    }
            ...
            return new CharProperty() {
                boolean isSatisfiedBy(int ch) {
                    if (inRange(lower, ch, upper))
                        return true;
                    int up = Character.toUpperCase((char)ch);
                    return inRange(lower, up, upper) ||
                            inRange(lower, Character.toLowerCase((char)up), upper);}};
        return new CharProperty() {
            boolean isSatisfiedBy(int ch) {
                return inRange(lower, ch, upper) ||
                        ASCII.isAscii(ch) &&
    ...
    static class CIBackRef extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            for (int index=0; index<groupSize; index++) {
                ...
                if (c1 != c2) {
                    if (doUnicodeCase) {
                        int cc1 = Character.toUpperCase((char)c1);
                        int cc2 = Character.toUpperCase((char)c2);
                        if (cc1 != cc2 &&
                                Character.toLowerCase((char)cc1) !=
                                        Character.toLowerCase((char)cc2))
                            return false;
                    } else {
    ...
    static final class Bound extends Node {
        ...
        boolean isWord(int ch) {
            return useUWORD ? UnicodeProp.WORD.is(ch)
                    : (ch == '_' || Utils.isLetterOrDigit((char) ch));
        }
        ...
        int check(Matcher matcher, int i, CharSequence seq) {
            ...
            if (i > startIndex) {
                ...
                left = (isWord(ch) ||
                        ((CharacterDataLatin1.instance.getType(ch) == CharacterData.NON_SPACING_MARK)
                                            ...
                                            CharSequence seq)
    {
        ...
        for (int x=i; x >= start; x--) {
            ...
            if (Utils.isLetterOrDigit((char)ch))
                return true;
            if (CharacterDataLatin1.instance.getType(ch) == CharacterData.NON_SPACING_MARK)
                continue;
            return false;
        }
        return false;
